const projects = [
  {
    ...
    gallery: ['svDCgUDjx0ldwrr9WvDCHBBDhVY.jpg','leSCheyI5VnbEV7C1Q1PCPBHvkw.png','PgR08AAWAlj2UAIQdDPozDt3Zc.png','xl6TVulMoSxPobCHbOKKkzNVVs.jpg']
  },
  {
  ...
  },
  {
    ...
    gallery: ['xTcZCVunWNKAOLmePnJ0mLJtSdY.jpg','bxyDYSIkg5Qh5xEmf1TedH8iOxU.jpg','e1f2Y2r11hn6d8YUvKwN3ylL27E.jpg','M2qW4a6vPViqWEJZiZAZkfecmI.jpg']
  },
  {
...
const asset = name => name.includes('/') ? name : `https://framerusercontent.com/images/${name}?width=1800`;
...
projects.forEach(project => {
  ...
  article.innerHTML = `
    <div class="project-media" style="background-image:url('${project.cover}')"></div>
...
function openProject(project) {
  content.innerHTML = `
    <header class="project-hero" style="background-image:url('${project.cover}')">
      <div><p class="eyebrow">${project.category} · ${project.year}</p><h2>${project.title}</h2></div>
    </header>
  ...
  <div class="project-story-media">
    ...
    ${project.video
      ? `<video autoplay muted loop playsinline poster="${project.storyPoster || project.cover}">
           <source src="${project.video}" type="video/mp4">
         </video>`
      : `<div class="project-story-still"
  ...
  <div class="project-story-copy">
    ...
    <section class="filmstrip filmstrip--${project.slug}" aria-label="${project.title} gallery">
      ${project.gallery.map((image, i) => `<img src="${asset(image)}" alt="${project.title} — selected image ${i + 1}" loading="lazy">`).join('')}
    </section>`;
  dialog.showModal();
  dialog.scrollTop = 0;
}
